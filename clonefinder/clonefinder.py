...
def clonefinder():
    ...
    if parser.parse() == False:
        ...
        if status != True:
            print('CloneFinder cannot make inferences, because samples do not have'
            ...
            while Repeat == 'y' and ItC <= 5:
                ...
                if Message[:10] != 'decomposed' or DecomTuNum > (1.0 * len(v_obs) / 2):
                    ...
                    if status == True and ItC < 5:
                        # True will execute code to remove redundant seqs (True is default)
                        decomseqs_with_ancestor, A1 = \
                            tree_builder.alignment_least_back_parallel_muts()
                        ...
                        decomseqs_with_ancestor_clone_freq =  \
                            clone_frequency_cnv2.Tumor2Clone_frequency
                        ...
                        print('decomposed clones do not have evolutionary structure.'
                              ' So, we will not decompose them.')
                        summary_file += ('decomposed clone genotypes are not good'
                                         ' (no evolutionary structure), so clones were not'
                                         ' decomposed.\n')
                        Repeat = 'n'
                        final_seq = clone_frequency_cnv.hitclone_seq_builder
                        final_clofre = clone_frequency_cnv.Tumor2Clone_frequency
            ...
            if status == True:
                ...
                ancestor_states, offspring2ancestor, cell2code, code2cell = \
